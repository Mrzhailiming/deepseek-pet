 ...
 *
 * 用法（零依赖，只需要 node）：
 *   node pack.mjs                      # 打 ./dsh-pet-plugin，产物落在 ./dist
 *   node pack.mjs --check              # 只校验，不打包
 *   node pack.mjs 别的包目录            # 换要打的包
 *   node pack.mjs --out build          # 换输出目录
 *   node pack.mjs --packer npm         # 强制用 npm pack（默认 pnpm，失败自动退回 npm）
 */
...
/** 没给包目录时打这个。 */
const DEFAULT_PACKAGE_DIR = 'dsh-pet-plugin'
...
const outDir = resolve(here, option('--out', 'dist'))
...
const root = resolve(here, positional ?? DEFAULT_PACKAGE_DIR)
